...
try:_ = stopwords.words("english") _ = WordNetLemmatizer()
except Exception:
...
SEED = 42
np.random.seed(SEED)
def read_eml_file(path: Path) -> str: """Parse .eml file and return plain text (subject + body)."""
try:
...
if ctype == "text/plain":
try:
parts.append(part.get_payload(decode=True).decode(errors="ignor
e"))
except Exception:
continue
else:
try:
parts.append(msg.get_payload(decode=True).decode(errors="ignor
e"))
except Exception:
...
return "" def read_text_file(path: Path) -> str: """Read a plain text file."""
try:
return path.read_text(encoding="utf-8", errors="ignore")
except Exception:
return "" def load_dataset_from_folders(base_dir: str) -> Tuple[List[str],
List[int]]: """
Load dataset assuming directory structure:
base_dir/
ham/ -> contains ham emails (.eml or .txt)
spam/ -> contains spam emails (.eml or .txt)
Returns texts, labels (0 for ham, 1 for spam) """ base = Path(base_dir)
ham_dir = base / "ham"
spam_dir = base / "spam"
texts = []
labels = []
...
return texts, labels
# --- Preprocessing utilities --- RE_HTML = re.compile(r"<[^>]+>")
RE_NON_ALPHANUM = re.compile(r"[^\w\s]") # keep underscores
as word chars but OK
def clean_text(text: str) -> str: """Basic cleaning: remove html, lower, remove non-alpha,
collapse spaces."""
if not text:
return ""
...
return text
def tokenize_lemmatize(text: str) -> List[str]: """Tokenize and lemmatize, remove stopwords and short
tokens."""
...
output.append(lemma)
return output
def preprocess_texts(texts: List[str]) -> List[str]: """Apply cleaning + tokenization/lemmatization and re-join
tokens to a string (for TF-IDF).""" processed = []
for t in texts:
c = clean_text(t)
tok = tokenize_lemmatize(c)
processed.append(" ".join(tok))
return processed
# --- Training / evaluation pipeline --- def train_and_evaluate(X_train_vec, X_test_vec, y_train, y_test,
do_gridsearch=True):
results = {}
# Naive Bayes
nb = MultinomialNB()
...
results["NaiveBayes"] = { "model": nb, "time_s": t_nb, "accuracy": accuracy_score(y_test, ypred_nb), "precision": precision_score(y_test, ypred_nb), "recall": recall_score(y_test, ypred_nb), "f1": f1_score(y_test, ypred_nb), "y_pred": ypred_nb,
}
...
t_lr = time.time() - t0
best_lr = gs.best_estimator_ else:
t0 = time.time()
lr.fit(X_train_vec, y_train)
t_lr = time.time() - t0
best_lr = lr
ypred_lr = best_lr.predict(X_test_vec)
results["LogisticRegression"] = { "model": best_lr, "time_s": t_lr, "accuracy": accuracy_score(y_test, ypred_lr), "precision": precision_score(y_test, ypred_lr), "recall": recall_score(y_test, ypred_lr), "f1": f1_score(y_test, ypred_lr), "y_pred": ypred_lr,
}
# SVM (LinearSVC)
...
gs.fit(X_train_vec, y_train)
t_svm = time.time() - t0
best_svm = gs.best_estimator_ else:
t0 = time.time()
svm.fit(X_train_vec, y_train)
t_svm = time.time() - t0
best_svm = svm
ypred_svm = best_svm.predict(X_test_vec)
results["SVM"] = { "model": best_svm, "time_s": t_svm, "accuracy": accuracy_score(y_test, ypred_svm), "precision": precision_score(y_test, ypred_svm),
"recall": recall_score(y_test, ypred_svm), "f1": f1_score(y_test, ypred_svm), "y_pred": ypred_svm,
}
return results
...
out_path = Path(out_dir) / "model_accuracy_comparison.png" plt.tight_layout()
